...
class RadialBasisFunction(BasisFunction):
    ...
    def size(self):
        ...
        """
        return (len(self.means[0]) + 1) * self.num_actions
...
class MultiDimensionalPolynomialBasis(BasisFunction):
    ...
    def __init__(self, n_bfs, num_actions):
        """Initialize polynomial basis function."""
        self.__num_actions = BasisFunction._validate_num_actions(num_actions)

        if n_bfs < 0:
            raise ValueError('n_bfs must be >= 0')
        self.n_bfs = n_bfs
    ...
    def size(self):
        ...
        """
        return self.n_bfs * self.num_actions
    ...
    def evaluate(self, state, action):
        ...
        #v = np.append(state,action)
        #bf1 = 1
        #bf2 = np.dot(np.dot(v, Q1), v)
        #bf3 = np.dot(np.dot(v, Q2), v)
        #bf4 = np.dot(np.dot(v, Q3), v)

        phi[offset] = state[0]
        phi[offset+1] = state[1]
        phi[offset+2] = state[2]
        phi[offset+3] = state[3]
